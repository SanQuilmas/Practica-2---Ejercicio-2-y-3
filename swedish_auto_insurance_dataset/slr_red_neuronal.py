...
X = df.iloc[1:, 0].values.astype(np.float32)
y = df.iloc[1:, 1].values.astype(np.float32)
...
for i in range(len(y_pred_np)):
    ...
    if y_true_scalar in seen_nums:
        count = seen_nums.index(y_true_scalar)
    else:
        seen_nums.append(y_true_scalar)
        fansts.append([0, 0, 0, 0])
        count = len(fansts) - 1

    if(round(y_pred_scalar, 1) == round(y_true_scalar, 1)):
        fansts[count][0] = fansts[count][0] + 1
        for j in range(10):
            if((y_pred_scalar * 10) == j):
                pass
            else:
                fansts[j][1] = fansts[j][1] + 1

    elif(round(y_pred_scalar, 1) != round(y_true_scalar, 1)):
        fansts[count][3] = fansts[count][3] + 1
        seen_nums.append(y_true_scalar)
        fansts.append([0, 0, 0, 0])
        count = len(fansts) - 1
        fansts[count][2] = fansts[count][2] + 1
        for j in range((len(fansts) - 1)):
            if((y_pred_scalar * 10) == j or (y_true_scalar * 10) == j):
                pass
            else:
                fansts[j][1] = fansts[j][1] + 1
...
for i in range((len(fansts) - 1)):
    ...
    print(str(seen_nums[i]) + "\t" + str(round(sensitivity,2)) + "\t\t" + str(round(specificity, 2)) + "\t\t\t" + str(round(precision, 2)) + "\t" + str(round(f1_score, 2)))